...
if(gradTerm.includes("You")) { //"You have not applied to Graduate."
...
//Get majors/minors
const majorList = [];
$('b:contains("Major(s)")').next().each((index, element) => {
...
const minorList = [];
$('b:contains("Minor(s)")').next().each((index, element) => {
...
$('table.dhckDataWB:first').each((index, element) => {
    $(element).find('tr').each((ind, elem) => {
        if (ind != 0) { //skip the column names
            const classData = {};
