...
output[`field_year`] = field.slice(field.lastIndexOf("Catalog") - 8, field.indexOf("Catalog") - 1); // Catalog year
...
$('table.degReqTBL:first').find('tr').each((index, element) => {
    titleRow = $(element).find('b[style="font-size:16px;"]').text().trim(); // Checks for section title formatting
    if (titleRow != "") {                   // If section title found:
        if (req[`section_title`] != "") {   // Push the previous section title as an empty comment group
            req[`comment`] = true;
            reqs.push(req);
        }
        ...
        req[`classes`] = [];
        req[`comment`] = false;
    } else if ($(element).hasClass('degReqRowA') || $(element).hasClass('degReqRowA')) { // Otherwise, check the row for degReqRow classes
        $(element).children().each((ind, elem) => { // If found, iterate through each child of the row (add cases if more info needed):
            switch(ind) {
                case 0:                             // From the first, grab each class from the table child and put it in the class list.
                    $(elem).find('table').each((i, e) => {
